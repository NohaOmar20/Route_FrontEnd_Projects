...
async function loadMealNutrition(meal) {
    ...
    // Guard against null/failed API responses
    if (!nutrition) {
        let slot = document.getElementById("meal-details-nutrition-slot")
        if (slot) slot.innerHTML = `<p class="text-red-500 font-medium">Failed to load nutrition details. Please try again.</p>`
        
        document.getElementById("log-meal-btn-spinner").classList.add("hidden")
        document.getElementById("log-meal-btn-text").textContent = "Nutrition Unavailable"
        return
    }
    ...
    }, { once: true }) // Ensures the event listener isn't duplicated across multiple views
}
...
// Modal pop-up for logging meals
...
// Clear Today's Log Delete all logged food items for today
...
// Search by search button click or Enter key press
...
// Api fetching for products by search term
